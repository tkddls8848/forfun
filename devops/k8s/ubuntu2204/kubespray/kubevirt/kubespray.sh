...
#!/usr/bin/expect -f
spawn ssh-copy-id vagrant@192.168.60.10
expect {
...
#!/usr/bin/expect -f
spawn ssh-copy-id vagrant@192.168.60.2$i
expect {
    "Are you sure you want to continue connecting (yes/no" {
        send "yes\r"; exp_continue
    }
...
[all]
k8s-master ansible_host=192.168.60.10  ip=192.168.60.10
k8s-worker1 ansible_host=192.168.60.21  ip=192.168.60.21
k8s-worker2 ansible_host=192.168.60.22  ip=192.168.60.22
k8s-worker3 ansible_host=192.168.60.23  ip=192.168.60.23
