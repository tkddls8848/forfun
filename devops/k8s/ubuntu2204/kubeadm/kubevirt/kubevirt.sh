...
if egrep -c '(vmx|svm)' /proc/cpuinfo; then ## check for hardware virtualize
    ...
    curl -L https://github.com/kubevirt/kubevirt/releases/download/${KUBEVIRT_VERSION}/kubevirt-cr.yaml -o kubevirt-cr-no-kvm.yaml
    sed -i '/^\s*developerConfiguration:/a\ \ \ \ \ \ useEmulation: true' kubevirt-cr-no-kvm.yaml
    kubectl apply -f https://github.com/kubevirt/kubevirt/releases/download/${KUBEVIRT_VERSION}/kubevirt-cr-no-kvm.yaml
fi
