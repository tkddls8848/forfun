...
- 노드 간 작업은 직접 SSH 명령으로 처리하지 않는다. 단, `kvm/kubeadm_GPU`는 예외로 둔다.
- 노드 간 작업은 Ansible inventory에 master/worker/storage 역할을 정의하고, Ansible playbook이 각 대상 노드에서 실행하는 방식으로 통일한다. 단, `kvm/kubeadm_GPU`는 예외로 둔다.
...
- `kvm/kubeadm_GPU`는 예외로 둔다. 기존 VM 생성/SSH 기반 흐름은 유지하되, 위험한 cleanup과 latest 사용은 제거하고 필요한 이유를 문서화한다.
...
- [ ] `ubuntu/kubeadm`의 kubeadm 인자 계약을 수정한다.
  ...
  - 수정 요청: MicroK8s를 제외한 Kubernetes 클러스터의 CNI를 Flannel로 통일한다. `ubuntu/kubeadm`은 Calico 관련 인자를 제거하고, Ansible inventory 또는 group vars에서 Kubernetes 버전과 Flannel 버전을 명시해 넘긴다.
...
- [ ] 클러스터별 Ansible inventory를 추가한다.
  ...
  - 수정 요청: 클러스터별 `inventory.yml` 또는 `inventory.ini`를 추가하고 `control_plane`, `workers`, `storage` 같은 그룹을 정의한다. 노드 수, IP, 역할, 버전, 네트워크 대역 등 클러스터 설정 정보는 Ansible inventory 또는 group vars에서 단일 기준으로 관리한다. `kvm/kubeadm_GPU`는 예외로 두고 기존 스크립트 흐름을 문서화한다.


## 높은 우선순위의 정확성 수정

- [ ] `ubuntu/cephadm`와 `kvm/cephadm`의 차이를 명확히 문서화한다.
  - 대상:
    - `ubuntu/cephadm`
    - `kvm/cephadm`
  - 현재 문제: 두 클러스터 정의의 Ceph 스크립트와 부가 파일이 유사하지만, provider와 실행 환경 차이가 문서에서 명확히 드러나지 않는다.
  - 수정 요청: 두 구현은 별도 클러스터 정의로 유지한다. 다만 `ubuntu/cephadm`와 `kvm/cephadm`의 provider, VM 디스크 방식, 네트워크, inventory, 실행 전제 차이를 README 또는 각 클러스터 문서에 명확히 적는다.
...
- [ ] kubeadm 클러스터별 CNI를 하나로 명시한다.
  - 현재 상태: `ubuntu/kubeadm`은 Calico를 사용한다.
  - 현재 상태: `kvm/kubeadm_GPU`는 Flannel을 사용한다.
  - 수정 요청: MicroK8s를 제외한 Kubernetes 클러스터는 Flannel로 전환한다. `ubuntu/kubeadm`은 Calico를 제거하고 Flannel로 전환한다. `kvm/kubeadm_GPU`는 예외 원칙에 따라 기존 흐름은 유지하되 Flannel 매니페스트 버전만 고정한다. MicroK8s는 기본 CNI를 유지한다.
...
- [ ] MicroK8s/Kubeflow 버전과 네트워크 가정을 고정한다.
  - 대상:
    - `ubuntu/microk8s/kubeflow_gpu/03_microk8s_gpu_addon_install.sh`
    - `ubuntu/microk8s/kubeflow_gpu/04_juju_kubeflow_lite_install.sh`
...
- [ ] 깨진 한글 주석과 echo 메시지를 복구한다.
  - 대상: `devops/k8s` 하위 대부분의 Vagrantfile과 shell script
...
- [x] 생성물/로컬 상태에 대한 `.gitignore` 항목을 추가한다.
  ...
  - 추가 필요 패턴:
    ...
    - `devops/k8s/**/*.img`
    - `devops/k8s/**/*.iso`
    - `devops/k8s/**/kubeconfig`
    - `devops/k8s/**/kubeconfig-*`
    - `devops/k8s/**/admin.conf`
    - `devops/k8s/**/join-command.sh`
    - `devops/k8s/**/worker_join.sh`
    - `devops/k8s/**/*.retry`
    - `devops/k8s/**/.ansible/`
    - `devops/k8s/**/.cache/`
    - `devops/k8s/**/tmp/`
    - `devops/k8s/**/rook/`
    - `devops/k8s/**/kubespray-repo/`
  - 현재 추적 중인 정리 검토 대상:
    - `ubuntu/hadoop/tmp/core-site.xml`
  - 처리 결과: 루트 `.gitignore`에 `devops/k8s` 경로 한정 패턴을 추가했다. 기존 `.vagrant/`, `**/.claude`, `password.rb` 중복 추가는 피했다.
...
- [x] `ubuntu/microk8s/kubeflow_gpu/destroy_cluster.sh` 오타를 수정한다.
  - 처리 결과: `destory_cluster.sh`를 `destroy_cluster.sh`로 변경했다.

- [x] `devops/k8s` 최상위 README를 추가한다.
  - 포함 내용:
    - 클러스터 목록
    - 클러스터별 지원 provider
    - 필요한 host 도구
    ...
    - 생성/삭제 명령
    - non-HA/lab-only 가정
  - 처리 결과: `devops/k8s/README.md`를 추가하고 각 클러스터의 목적, 지원 provider, 네트워크, inventory 위치, 생성/삭제 명령, known limitations를 정리했다. `kvm/kubeadm_GPU`는 예외 클러스터임을 README에도 명시했다.

- [x] 검증 진입점을 추가한다.
  - 처리 결과: `devops/k8s/validate.ps1`를 추가했다. shellcheck, `vagrant validate`, `ansible-inventory --list`를 설치된 도구 기준으로 실행한다.

## 클러스터별 처리 방향

- [ ] `ubuntu/kubeadm`: 사용 전 수정 필수. 스크립트 경로 불일치와 누락된 버전 인자가 있다.

- [ ] `ubuntu/kubespray`: inventory/node count를 단일 소스로 만들고 password SSH 자동화를 제거한 뒤 사용한다.

- [ ] `ubuntu/cephadm`: 별도 클러스터 정의로 유지하고, 위험한 SSH 설정과 디스크 wipe 동작을 제거하며, `.vagrant`를 삭제한다. `kvm/cephadm`와의 provider/디스크/네트워크 차이를 문서화한다.

- [ ] `kvm/cephadm`: 별도 클러스터 정의로 유지하고, libvirt 전용 디스크/provider 전제를 문서화하며, `.vagrant`와 로컬 설정을 삭제한다.

- [ ] `kvm/kubeadm_GPU`: 예외 클러스터로 둔다. 기존 VM 생성/SSH 기반 동작은 유지하되 주석과 README로 이유를 문서화한다. Flannel 버전을 고정하며, 파괴적 cleanup은 rollback에만 둔다.

- [ ] `centos8/minikube`: Minikube/KubeVirt 버전을 고정하고, OS별 차이를 문서화

- [ ] `centos8/cephfs`: `devops/k8s` 검토 대상에 포함한다. password SSH, sudoers append, expect 기반 Kubespray 자동화, Rook 예제 디렉터리 의존, 외부 디스크 경로 하드코딩을 제거하거나 Ansible inventory/group vars 기준으로 전환한다.

- [x] `rocky/minikube`: `centos8/minikube` OS별 차이를 문서화

- [ ] `ubuntu/kubeadm/kubevirt`: KubeVirt 버전을 고정하고 GitHub API로 `latest`를 조회하는 방식을 제거한다.

- [ ] `ubuntu/microk8s/kubeflow_gpu`: channel/version을 고정하고, 네트워크 대역을 클러스터 정의값으로 관리하며, 일반 설치 흐름에서 광범위한 home/cache 삭제를 제거한다.

- [ ] `ubuntu/k3s`: pipe-to-shell latest installer를 제거하고, 고정된 installer 버전과 명시 설치 옵션을 사용한다.

- [ ] `ubuntu/hadoop`: Kubernetes 클러스터 정의가 아니라면 `k8s` 하위에서 이동한다. 유지한다면 목적과 실행 방식을 명확히 문서화한다.
