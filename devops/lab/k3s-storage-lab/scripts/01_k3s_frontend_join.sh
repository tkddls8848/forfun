#!/bin/bash
# Phase 2 (Packer AMI): Frontend — k3s server 서비스 등록 + agent × 2 조인
# 전제: k3s 바이너리는 AMI에 사전 설치됨 (INSTALL_K3S_SKIP_DOWNLOAD=true)
# 실행: ssh ubuntu@<FRONTEND_IP> 'sudo bash -s' < 01_k3s_frontend_join.sh
set -e
