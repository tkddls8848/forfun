#!/bin/bash
# Phase 3 (Packer AMI): Backend — Ceph 클러스터 구성만 실행
# 전제: python3, podman, cephadm, nvme-cli 는 AMI에 사전 설치됨
# 실행: ssh ubuntu@<BACKEND_IP> 'sudo bash -s' < 02_ceph_bootstrap_only.sh
set -e
