#!/bin/bash
# Phase 4 (Packer AMI): Backend — BeeGFS conf 및 서비스 기동만 실행
# 전제: beegfs-mgmtd, beegfs-meta, beegfs-storage, xfsprogs, lvm2 는 AMI에 사전 설치됨
# 실행: ssh ubuntu@<BACKEND_IP> 'sudo bash -s' < 03_beegfs_conf_only.sh
set -e
